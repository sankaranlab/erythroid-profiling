...
gata1inserts <- rbind(
  getInsertScores("P1", "CTCF"),
  getInsertScores("P2", "CTCF"),
  getInsertScores("P4", "CTCF"),
  getInsertScores("P8", "CTCF"))
...
p1 <- ggplot(gata1inserts, aes(x = position, y = value, color = population, group = population)) +
  ...
  pretty_plot(fontsize = 8) + L_border() + labs(x = "Position relative to center of NFE2 Motif",
                                                y = "Normalized Tn5 Insertions", color = "") +
  theme(legend.position = "none") 

cowplot::ggsave(p1, file = "../../plots/CTCF-inserts.pdf", width = 2, height = 2)